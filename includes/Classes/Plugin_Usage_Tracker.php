...
class Plugin_Usage_Tracker
{
    ...
    public function get_data()
    {
        ...
        if (empty($plugin)) {
            ...
            $body['message'] .= __('We can\'t detect any plugin information. This is most probably because you have not included the code in the plugin main file.', 'px-elements');
    ...
    public function optin_notice()
    {
        ...
        if ($is_local) {
            ...
            if ($this->marketing != 1) {
                ...
                $notice_text = __('Thank you for installing our plugin. We would like to track its usage on your site. We don\'t record any sensitive data, only information regarding the WordPress environment and plugin settings, which we will use to help us make improvements to the plugin. Tracking is completely optional.', 'px-elements');
            } else {
                // If we have option 1 for marketing, we include reference to sending product information here
                $notice_text = __('Want to help make <strong>Essential Addons for Elementor</strong> even more awesome? You can get a <strong>10% discount coupon</strong> for Pro upgrade if you allow. <a class="insights-data-we-collect" href="#">What we collect.</a>', 'px-elements');
            }
			...
			<div class="notice notice-info updated put-dismiss-notice">
				...
				<p>
					<a href="<?php echo esc_url($url_yes); ?>" class="button-primary"><?php _e('Sure, I\'d like to help', 'px-elements');?></a>
					<a href="<?php echo esc_url($url_no); ?>" class="button-secondary"><?php _e('No Thanks', 'px-elements');?></a>
				</p>
    ...
    public function marketing_notice()
    {
        ...
        if (isset($_GET['marketing_optin'])) {
            ...
            $marketing_text = __('Thank you for opting in to tracking. Would you like to receive occasional news about this plugin, including details of new features and special offers?', 'px-elements');
			...
			<div class="notice notice-info updated put-dismiss-notice">
				...
				<p>
					<a href="<?php echo esc_url($url_yes); ?>" data-putnotice="yes" class="button-secondary"><?php _e('Yes Please', 'px-elements');?></a>
					<a href="<?php echo esc_url($url_no); ?>" data-putnotice="no" class="button-secondary"><?php _e('No Thank You', 'px-elements');?></a>
				</p>
			</div>
			<?php }
    }
    ...
    {
        $form = array();
        $form['heading'] = __('Sorry to see you go', 'px-elements');
        $form['body'] = __('Before you deactivate the plugin, would you quickly give us your reason for doing so?', 'px-elements');

        $form['options'] = array(
            __('I no longer need the plugin', 'px-elements'),
            [
                'label' => __('I found a better plugin', 'px-elements'),
                'extra_field' => __('Please share which plugin', 'px-elements'),
            ],
            __("I couldn't get the plugin to work", 'px-elements'),
            __('It\'s a temporary deactivation', 'px-elements'),
            __('I have ' . $this->pro_plugin_name, 'px-elements'),
            [
                'label' => __('Other', 'px-elements'),
                'extra_field' => __('Please share the reason', 'px-elements'),
                'type' => 'textarea',
            ],
        );
    ...
    public function goodbye_ajax()
    {
        ...
        $html .= '<p class="deactivating-spinner"><span class="spinner"></span> ' . __('Submitting form', 'px-elements') . '</p>';
        ?>
		<style type="text/css">
		...
		<script>
			jQuery(document).ready(function($){
				$("#wpdev-put-goodbye-link-<?php echo esc_attr($this->plugin_name); ?>").on("click",function(){
					...
					$("#wpdev-put-goodbye-form-<?php echo esc_attr($this->plugin_name); ?>").html( '<?php echo $html; ?>' + '<div class="wpdev-put-goodbye-form-footer"><div class="wpdev-put-goodbye-form-buttons"><a id="put-submit-form-<?php echo esc_attr($this->plugin_name); ?>" class="eael-put-submit-btn" href="#"><?php _e('Submit and Deactivate', 'px-elements');?></a>&nbsp;<a class="eael-put-deactivate-btn" href="'+url+'"><?php _e('Just Deactivate', 'px-elements');?></a></div></div>');
