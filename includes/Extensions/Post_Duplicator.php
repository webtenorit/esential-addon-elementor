...
class Post_Duplicator
{
    ...
    public function admin_bar_menu( $wp_admin_bar ){
        ...
        $wp_admin_bar->add_menu( 
            array( 
                'id' => 'eae-duplicator', 
                'title' => __( 'EA Duplicator', 'pixerex-elements'),
                'href' => $duplicate_url
            ) 
        );
    }
    ...
    public function row_actions( $actions, $post ) {
        ...
        if( current_user_can('edit_posts') && ($enabled_on == 'all' || $post->post_type == $enabled_on)) {
            $duplicate_url = admin_url('admin.php?action=eae_duplicate&post=' . $post->ID );
            $duplicate_url = wp_nonce_url( $duplicate_url, 'ea_duplicator' );
            $actions['eae_duplicate'] = sprintf( '<a href="%s" title="%s">%s</a>', $duplicate_url,  __( 'Duplicate ' . $post->post_title, 'pixerex-elements'), __( 'EA Duplicator', 'pixerex-elements') );
        }
        return $actions;
    }
    /**
