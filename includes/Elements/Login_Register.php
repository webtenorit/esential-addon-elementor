...
class Login_Register extends Widget_Base {
	...
	public function get_title() {
		return esc_html__( 'Login | Register Form', 'pixerex-elements' );
	}
	...
	protected function get_form_field_types() {
		return apply_filters( 'eael/registration-form-fields', [
			'user_name'    => __( 'Username', 'pixerex-elements' ),
			'email'        => __( 'Email', 'pixerex-elements' ),
			'password'     => __( 'Password', 'pixerex-elements' ),
			'confirm_pass' => __( 'Confirm Password', 'pixerex-elements' ),
			'first_name'   => __( 'First Name', 'pixerex-elements' ),
			'last_name'    => __( 'Last Name', 'pixerex-elements' ),
			'website'      => __( 'Website', 'pixerex-elements' ),
		] );
	}
	...
	protected function init_content_general_controls() {
		...
		$this->add_control( 'default_form_type_notice', [
			...
			'raw'             => __( 'Choose the type of form you want to show by default. Note: you can show both forms in a single page even if you select only login or registration from below.', 'pixerex-elements' ),
			'content_classes' => 'elementor-panel-alert elementor-panel-alert-info',
		] );
		$this->add_control( 'default_form_type', [
			'label'   => __( 'Default Form Type', 'pixerex-elements' ),
			'type'    => Controls_Manager::SELECT,
			'options' => [
				'login'    => __( 'Login', 'pixerex-elements' ),
				'register' => __( 'Registration', 'pixerex-elements' ),
			],
			'default' => 'login',
		] );
		if ( ! $this->user_can_register ) {
			$this->add_control( 'registration_off_notice', [
				...
				'raw'             => sprintf( __( 'Registration is disabled on your site. Please enable it to use registration form. You can enable it from Dashboard » Settings » General » %1$sMembership%2$s.', 'pixerex-elements' ), '<a href="' . esc_attr( esc_url( admin_url( 'options-general.php' ) ) ) . '" target="_blank">', '</a>' ),
		...
		$this->add_control( 'hide_for_logged_in_user', [
			'label'   => __( 'Hide all Forms from Logged-in Users', 'pixerex-elements' ),
			'type'    => Controls_Manager::SWITCHER,
			'default' => 'yes',
		] );
		$this->add_control( 'gen_lgn_content_po_toggle', [
			'label'        => __( 'Login Form General', 'pixerex-elements' ),
			'type'         => Controls_Manager::POPOVER_TOGGLE,
			'label_off'    => __( 'Controls', 'pixerex-elements' ),
		...
		$this->add_control( 'show_log_out_message', [
			'label'   => __( 'Show Logout Link', 'pixerex-elements' ),
			'type'    => Controls_Manager::SWITCHER,
			'default' => 'yes',
		] );
		$this->add_control( 'show_lost_password', [
			'label'   => __( 'Show Lost your password?', 'pixerex-elements' ),
			'type'    => Controls_Manager::SWITCHER,
			'default' => 'yes',
		] );

		$this->add_control( 'lost_password_text', [
			'label'       => __( 'Lost Password Text', 'pixerex-elements' ),
			'label_block' => true,
			'type'        => Controls_Manager::TEXT,
			'dynamic'     => [
				'active' => true,
			],
			'default'     => __( 'Forgot password?', 'pixerex-elements' ),
			'condition'   => [
				'show_lost_password' => 'yes',
			],
		] );
		$this->add_control( 'lost_password_link_type', [
			'label'       => __( 'Lost Password Link to', 'pixerex-elements' ),
			'label_block' => true,
			'type'        => Controls_Manager::SELECT,
			'options'     => [
				'default' => __( 'Default WordPress Page', 'pixerex-elements' ),
				'custom'  => __( 'Custom URL', 'pixerex-elements' ),
			],
			'default'     => 'default',
			'condition'   => [
				'show_lost_password' => 'yes',
		...
		$this->add_control( 'lost_password_url', [
			'label'         => __( 'Custom Lost Password URL', 'pixerex-elements' ),
			'label_block'   => true,
		...
		if ( $this->user_can_register ) {
			...
			$this->add_control( 'show_register_link', [
				'label'     => __( 'Show Register Link', 'pixerex-elements' ),
				'type'      => Controls_Manager::SWITCHER,
				'default'   => 'yes',
				'separator' => 'before',
			...
			$this->add_control( 'registration_link_text', [
				'label'       => __( 'Register Link Text', 'pixerex-elements' ),
				'label_block' => true,
				'description' => __( 'You can put text in two lines to make the last line linkable. Pro Tip: You can keep the first line empty and put the text only in the second line to get a link only.', 'pixerex-elements' ),
				'type'        => Controls_Manager::TEXTAREA,
				'rows'        => 2,
				'dynamic'     => [
					'active' => true,
				],
				'default'     => __( " \nRegister Now", 'pixerex-elements' ),
				'condition'   => [
					'show_register_link' => 'yes',
				],
			] );
			$this->add_control( 'registration_link_action', [
				'label'       => __( 'Registration Link Action', 'pixerex-elements' ),
				'label_block' => true,
				'type'        => Controls_Manager::SELECT,
				'options'     => [
					'default' => __( 'WordPress Registration Page', 'pixerex-elements' ),
					'custom'  => __( 'Custom URL', 'pixerex-elements' ),
					'form'    => __( 'Show Register Form', 'pixerex-elements' ),
				],
				'default'     => 'form',
				'condition'   => [
					'show_register_link' => 'yes',
			...
			$this->add_control( 'custom_register_url', [
				'label'         => __( 'Custom Register URL', 'pixerex-elements' ),
				'label_block'   => true,
				'type'          => Controls_Manager::URL,
				'show_external' => false,
				'dynamic'       => [
		...
		$this->add_control( 'enable_login_recaptcha', [
			'label'        => __( 'Enable Google reCAPTCHA', 'pixerex-elements' ),
			'description'  => __( 'reCAPTCHA will prevent spam login from bots.', 'pixerex-elements' ),
			'type'         => Controls_Manager::SWITCHER,
		...
		if ( empty( $this->recaptcha_sitekey ) ) {
			$this->add_control( 'eael_login_recaptcha_keys_missing', [
				'type'            => Controls_Manager::RAW_HTML,
				'raw'             => sprintf( __( 'reCAPTCHA API keys are missing. Please add them from %sDashboard >> Essential Addons >> Elements >> Login | Register Form %sSettings', 'pixerex-elements' ), '<strong>', '</strong>' ),
		...
		if ( $this->user_can_register ) {
			$this->add_control( 'gen_reg_content_po_toggle', [
				'label'        => __( 'Register Form General', 'pixerex-elements' ),
				'type'         => Controls_Manager::POPOVER_TOGGLE,
				'label_off'    => __( 'Default', 'pixerex-elements' ),
				'label_on'     => __( 'Custom', 'pixerex-elements' ),
				'return_value' => 'yes',
				'default'      => 'yes',
			] );
			...
			$this->add_control( 'show_login_link', [
				'label'   => __( 'Show Login Link', 'pixerex-elements' ),
				'type'    => Controls_Manager::SWITCHER,
				'default' => 'yes',
			] );
			$this->add_control( 'login_link_text', [
				'label'       => __( 'Login Link Text', 'pixerex-elements' ),
				'label_block' => true,
				'description' => __( 'You can put text in two lines to make the last line linkable. Pro Tip: You can keep the first line empty and put the text only in the second line to get a link only.', 'pixerex-elements' ),
				'type'        => Controls_Manager::TEXTAREA,
				'rows'        => 2,
				...
				'default'     => __( " \nSign In", 'pixerex-elements' ),
				'condition'   => [
					'show_login_link' => 'yes',
				],
			] );
			$this->add_control( 'login_link_action', [
				'label'       => __( 'Login Link Action', 'pixerex-elements' ),
				'label_block' => true,
				'type'        => Controls_Manager::SELECT,
				'options'     => [
					'default' => __( 'Default WordPress Page', 'pixerex-elements' ),
					'custom'  => __( 'Custom URL', 'pixerex-elements' ),
					'form'    => __( 'Show Login Form', 'pixerex-elements' ),
				],
				'default'     => 'form',
				'condition'   => [
					'show_login_link' => 'yes',
			...
			$this->add_control( 'custom_login_url', [
				'label'         => __( 'Custom Login URL', 'pixerex-elements' ),
				'label_block'   => true,
				'show_external' => false,
				'type'          => Controls_Manager::URL,
				'dynamic'       => [
			...
			$this->add_control( 'enable_register_recaptcha', [
				'label'        => __( 'Enable Google reCAPTCHA', 'pixerex-elements' ),
				'description'  => __( 'reCAPTCHA will prevent spam registration from bots.', 'pixerex-elements' ),
				'type'         => Controls_Manager::SWITCHER,
				'label_on'     => __( 'Yes', 'pixerex-elements' ),
				'label_off'    => __( 'No', 'pixerex-elements' ),
				'return_value' => 'yes',
	...
	protected function init_content_login_fields_controls() {
		...
		$this->add_control( 'login_user_label', [
			'label'       => __( 'Username Label', 'pixerex-elements' ),
			'placeholder' => __( 'Username or Email Address', 'pixerex-elements' ),
			'default'     => __( 'Username or Email Address', 'pixerex-elements' ),
			'type'        => Controls_Manager::TEXT,
			'dynamic'     => [ 'active' => true, ],
			'label_block' => true,
			'condition'   => [ 'login_label_types' => 'custom', ],
		...
		$this->add_control( 'login_password_label', [
			'label'       => __( 'Password Label', 'pixerex-elements' ),
			'placeholder' => __( 'Password', 'pixerex-elements' ),
			'default'     => __( 'Password', 'pixerex-elements' ),
			'type'        => Controls_Manager::TEXT,
			'dynamic'     => [ 'active' => true, ],
			'label_block' => true,
			'condition'   => [ 'login_label_types' => 'custom', ],
		...
		$this->add_control( 'login_placeholders_heading', [
			'label'     => esc_html__( 'Placeholders', 'pixerex-elements' ),
			'type'      => Controls_Manager::HEADING,
			'condition' => [ 'login_label_types' => 'custom', ],
			'separator' => 'before',
		] );
		...
		$this->add_control( 'login_user_placeholder', [
			'label'       => __( 'Username Placeholder', 'pixerex-elements' ),
			'placeholder' => __( 'Username or Email Address', 'pixerex-elements' ),
			'default'     => __( 'Username or Email Address', 'pixerex-elements' ),
			'type'        => Controls_Manager::TEXT,
			'dynamic'     => [ 'active' => true, ],
		...
		$this->add_control( 'login_password_placeholder', [
			'label'       => __( 'Password Placeholder', 'pixerex-elements' ),
			'placeholder' => __( 'Password', 'pixerex-elements' ),
			'default'     => __( 'Password', 'pixerex-elements' ),
			'type'        => Controls_Manager::TEXT,
			'dynamic'     => [ 'active' => true, ],
		...
		$this->add_responsive_control( 'login_field_width', [
			'label'      => esc_html__( 'Input Fields width', 'pixerex-elements' ),
		...
		$this->add_control( 'login_show_remember_me', [
			'label'     => __( 'Remember Me Checkbox', 'pixerex-elements' ),
			'type'      => Controls_Manager::SWITCHER,
			'default'   => 'yes',
		...
		$this->add_control( 'password_toggle', [
			'label'     => __( 'Password Visibility Icon', 'pixerex-elements' ),
			'type'      => Controls_Manager::SWITCHER,
			'label_off' => __( 'Hide', 'pixerex-elements' ),
			'label_on'  => __( 'Show', 'pixerex-elements' ),
			'default'   => 'yes',
		...
		$this->add_control( 'login_button_heading', [
			'label'     => esc_html__( 'Login Button', 'pixerex-elements' ),
			'type'      => Controls_Manager::HEADING,
			'separator' => 'before',
		] );
		...
		$this->add_control( 'login_button_text', [
			...
			'type'        => Controls_Manager::TEXT,
			'dynamic'     => [ 'active' => true, ],
			'default'     => __( 'Log In', 'pixerex-elements' ),
			'placeholder' => __( 'Log In', 'pixerex-elements' ),
		] );
	...
	protected function init_form_header_controls() {
		$this->start_controls_section( 'section_content_lr_form_header', [
			'label' => __( 'Form Header Content', 'pixerex-elements' ),
		] );

		$this->add_control( 'lr_form_image', [
			'label'   => __( 'Form Header Image', 'pixerex-elements' ),
		...
		$this->add_control( "lr_form_image_position", [
			'label'     => __( 'Header Image Position', 'pixerex-elements' ),
		...
		$this->add_control( 'lr_form_logo', [
			'label'   => __( 'Form Header Logo', 'pixerex-elements' ),
			'type'    => Controls_Manager::MEDIA,
			'dynamic' => [
				'active' => true,
			],
		...
		$this->add_control( "lr_form_logo_position", [
			'label'     => __( 'Form Logo Position', 'pixerex-elements' ),
			'type'      => Controls_Manager::CHOOSE,
			'options'   => [
				'inline' => [
					'title' => __( 'Left', 'pixerex-elements' ),
		...
		$this->add_control( 'login_form_title', [
			'label'       => __( 'Login Form Title', 'pixerex-elements' ),
			'type'        => Controls_Manager::TEXT,
			'dynamic'     => [ 'active' => true, ],
			'placeholder' => __( 'Welcome Back!', 'pixerex-elements' ),
			'separator'   => 'before',
		] );
		$this->add_control( 'login_form_subtitle', [
			'label'       => __( 'Login Form Sub Title', 'pixerex-elements' ),
			'type'        => Controls_Manager::TEXTAREA,
			'dynamic'     => [ 'active' => true, ],
			'placeholder' => __( 'Please login to your account', 'pixerex-elements' ),
		] );

		$this->add_control( 'register_form_title', [
			'label'       => __( 'Register Form Title', 'pixerex-elements' ),
			'type'        => Controls_Manager::TEXT,
			'dynamic'     => [ 'active' => true, ],
			'placeholder' => __( 'Create a New Account', 'pixerex-elements' ),
			'separator'   => 'before',
		] );
		$this->add_control( 'register_form_subtitle', [
			'label'       => __( 'Register Form Sub Title', 'pixerex-elements' ),
			'type'        => Controls_Manager::TEXTAREA,
			'dynamic'     => [ 'active' => true, ],
			'placeholder' => __( 'Create an account to enjoy awesome features.', 'pixerex-elements' ),
		] );
	...
	protected function init_content_login_options_controls() {
		...
		$this->start_controls_section( 'section_content_login_options', [
			'label'      => __( 'Login Form Options', 'pixerex-elements' ),
			'conditions' => $this->get_form_controls_display_condition( 'login' ),
		] );

		$this->add_control( 'redirect_after_login', [
			'label' => __( 'Redirect After Login', 'pixerex-elements' ),
			'type'  => Controls_Manager::SWITCHER,
		] );
		...
		$this->add_control( 'redirect_url', [
			...
			'description'   => __( 'Please note that only your current domain is allowed here to keep your site secure.', 'pixerex-elements' ),
			'condition'     => [
		...
		//	'label' => __( 'Redirect After Logout', 'pixerex-elements' ),
		//	'type'  => Controls_Manager::SWITCHER,
		//] );
		//
		...
		//	'show_label'    => false,
		//	'show_external' => false,
		//	'placeholder'   => __( 'https://your-link.com', 'pixerex-elements' ),
		//	'description'   => __( 'Please note that only your current domain is allowed here to keep your site secure.', 'pixerex-elements' ),
		//	'condition'     => [
	...
	protected function init_content_terms_controls() {
		...
		$this->add_control( 'acceptance_label', [
			'label'       => __( 'Acceptance Label', 'pixerex-elements' ),
			'description' => __( 'Eg. I accept the terms & conditions. Note: First line is checkbox label & Last line will be used as link text.', 'pixerex-elements' ),
			'type'        => Controls_Manager::TEXTAREA,
			'rows'        => 2,
			'label_block' => true,
			'placeholder' => __( 'I Accept the Terms and Conditions.', 'pixerex-elements' ),
			/* translators: \n means new line. So, Don't translate this*/
			'default'     => __( "I Accept\n the Terms and Conditions.", 'pixerex-elements' ),
			'condition'   => [
		...
		$this->add_control( 'acceptance_text_source', [
			'label'     => __( 'Content Source', 'pixerex-elements' ),
			'type'      => Controls_Manager::SELECT,
			'options'   => [
				'editor' => __( 'Editor', 'pixerex-elements' ),
				'custom' => __( 'Custom', 'pixerex-elements' ),
			],
			'default'   => 'custom',
			'condition' => [
		...
		$this->add_control( 'acceptance_text', [
			'label'     => __( 'Terms and Conditions', 'pixerex-elements' ),
			'type'      => Controls_Manager::WYSIWYG,
			'rows'      => 3,
			'default'   => __( 'Please go through the following terms and conditions carefully.', 'pixerex-elements' ),
			'condition' => [
				'show_terms_conditions'  => 'yes',
		...
		$this->add_control( 'acceptance_text_url', [
			'label'       => __( 'Terms & Conditions URL', 'pixerex-elements' ),
			'description' => __( 'Enter the link where your terms & condition or privacy policy is found.', 'pixerex-elements' ),
			'type'        => Controls_Manager::URL,
			'dynamic'     => [
				'active' => true,
			],
	...
	protected function init_content_validation_messages_controls() {
		$this->start_controls_section( 'section_content_errors', [
			'label' => __( 'Validation Messages', 'pixerex-elements' ),
		] );

		$this->add_control( 'err_message_heading', [
			'label' => esc_html__( 'Error Messages', 'pixerex-elements' ),
			'type'  => Controls_Manager::HEADING,
		] );

		$this->add_control( 'err_email', [
			'label'       => __( 'Invalid Email', 'pixerex-elements' ),
			'type'        => Controls_Manager::TEXT,
			'label_block' => true,
			'placeholder' => __( 'Eg. Your email is invalid.', 'pixerex-elements' ),
			'default'     => __( "You have used an invalid email", 'pixerex-elements' ),
		] );
		$this->add_control( 'err_email_missing', [
			'label'       => __( 'Email is missing', 'pixerex-elements' ),
			'type'        => Controls_Manager::TEXT,
			'label_block' => true,
			'placeholder' => __( 'Eg. Email is missing or Invalid', 'pixerex-elements' ),
			'default'     => __( 'Email is missing or Invalid', 'pixerex-elements' ),
		] );
		$this->add_control( 'err_email_used', [
			'label'       => __( 'Already Used Email', 'pixerex-elements' ),
			'type'        => Controls_Manager::TEXT,
			'label_block' => true,
			'placeholder' => __( 'Eg. Your email is already in use..', 'pixerex-elements' ),
			'default'     => __( 'The provided email is already registered with other account. Please login or reset password or use another email.', 'pixerex-elements' ),
		] );
		$this->add_control( 'err_username', [
			'label'       => __( 'Invalid Username', 'pixerex-elements' ),
			'type'        => Controls_Manager::TEXT,
			'label_block' => true,
			'placeholder' => __( 'Eg. Your username is invalid.', 'pixerex-elements' ),
			'default'     => __( "You have used an invalid username", 'pixerex-elements' ),
		] );
		$this->add_control( 'err_username_used', [
			'label'       => __( 'Username already in use', 'pixerex-elements' ),
			'type'        => Controls_Manager::TEXT,
			'label_block' => true,
			'placeholder' => __( 'Eg. Your username is already registered.', 'pixerex-elements' ),
			'default'     => __( 'Invalid username provided or the username already registered.', 'pixerex-elements' ),
		] );
		$this->add_control( 'err_pass', [
			'label'       => __( 'Invalid Password', 'pixerex-elements' ),
			'type'        => Controls_Manager::TEXT,
			'label_block' => true,
			'placeholder' => __( 'Eg. Your password is invalid', 'pixerex-elements' ),
			'default'     => __( "Your password is invalid.", 'pixerex-elements' ),
		] );

		$this->add_control( 'err_conf_pass', [
			'label'       => __( 'Invalid Password Confirmed', 'pixerex-elements' ),
			'type'        => Controls_Manager::TEXT,
			'label_block' => true,
			'placeholder' => __( 'Eg. Password did not matched', 'pixerex-elements' ),
			'default'     => __( "Your confirmed password did not match", 'pixerex-elements' ),
		] );

		$this->add_control( 'err_loggedin', [
			'label'       => __( 'Already Logged In', 'pixerex-elements' ),
			'type'        => Controls_Manager::TEXT,
			'label_block' => true,
			'placeholder' => __( 'Eg. You are already logged in', 'pixerex-elements' ),
			'default'     => __( "You are already logged in", 'pixerex-elements' ),
		] );

		$this->add_control( 'err_recaptcha', [
			'label'       => __( 'reCAPTCHA Failed', 'pixerex-elements' ),
			'type'        => Controls_Manager::TEXT,
			'label_block' => true,
			'placeholder' => __( 'Eg. Recaptcha Validation Failed', 'pixerex-elements' ),
			'default'     => __( "You did not pass recaptcha challenge.", 'pixerex-elements' ),
		] );

		$this->add_control( 'err_tc', [
			'label'       => __( 'Terms & Condition Error', 'pixerex-elements' ),
			'type'        => Controls_Manager::TEXT,
			'label_block' => true,
			'placeholder' => __( 'Eg. You must accept the Terms & Conditions', 'pixerex-elements' ),
			'default'     => __( 'You did not accept the Terms and Conditions. Please accept it and try again.', 'pixerex-elements' ),
		] );

		$this->add_control( 'err_unknown', [
			'label'       => __( 'Other Errors', 'pixerex-elements' ),
			'type'        => Controls_Manager::TEXT,
			'label_block' => true,
			'placeholder' => __( 'Eg. Something went wrong', 'pixerex-elements' ),
			'default'     => __( "Something went wrong!", 'pixerex-elements' ),
		] );

		$this->add_control( 'success_message_heading', [
			'label'     => esc_html__( 'Success Messages', 'pixerex-elements' ),
			'type'      => Controls_Manager::HEADING,
			'separator' => 'before',
		] );

		$this->add_control( 'success_login', [
			'label'       => __( 'Successful Login', 'pixerex-elements' ),
			'type'        => Controls_Manager::TEXT,
			'label_block' => true,
			'placeholder' => __( 'Eg. You have logged in successfully', 'pixerex-elements' ),
			'default'     => __( "You have logged in successfully", 'pixerex-elements' ),
		] );
		$this->add_control( 'success_register', [
			'label'       => __( 'Successful Registration', 'pixerex-elements' ),
			'type'        => Controls_Manager::TEXTAREA,
			'default'     => __( 'Registration completed successfully, Check your inbox for password if you did not provided while registering.', 'pixerex-elements' ),
			'placeholder' => __( 'eg. Registration completed successfully', 'pixerex-elements' ),
		] );
	...
	protected function init_content_register_fields_controls() {
		...
		$this->start_controls_section( 'section_content_register_fields', [
			'label'      => __( 'Register Form Fields', 'pixerex-elements' ),
			'conditions' => $this->get_form_controls_display_condition( 'register' ),
		] );
		$this->add_control( 'register_form_field_note', [
			'type'            => Controls_Manager::RAW_HTML,
			'raw'             => __( 'Select the type of fields you want to show in the registration form', 'pixerex-elements' ),
			'content_classes' => 'elementor-panel-alert elementor-panel-alert-info',
		] );
		...
		$repeater->add_control( 'required', [
			'label'     => __( 'Required', 'pixerex-elements' ),
			'type'      => Controls_Manager::SWITCHER,
			'condition' => [
				'field_type!' => [
					'email',
		...
		$repeater->add_control( 'required_note', [
			'type'            => Controls_Manager::RAW_HTML,
			'raw'             => __( 'Note: This field is required by default.', 'pixerex-elements' ),
			'condition'       => [
				'field_type' => [
					'email',
					'password',
		...
		$repeater->add_responsive_control( 'width', [
			'label'   => __( 'Field Width', 'pixerex-elements' ),
			'type'    => Controls_Manager::SELECT,
			'options' => [
		...
		$this->add_control( 'register_fields', [
			...
			'default'     => [
				[
					...
					'field_label' => __( 'Username', 'pixerex-elements' ),
					'placeholder' => __( 'Username', 'pixerex-elements' ),
					'width'       => '100',
				],
				[
					'field_type'  => 'email',
					'field_label' => __( 'Email', 'pixerex-elements' ),
					'placeholder' => __( 'Email', 'pixerex-elements' ),
					'required'    => 'yes',
					'width'       => '100',
				],
				[
					'field_type'  => 'password',
					'field_label' => __( 'Password', 'pixerex-elements' ),
					'placeholder' => __( 'Password', 'pixerex-elements' ),
					'required'    => 'yes',
					'width'       => '100',
				],
		...
		$this->add_control( 'show_labels', [
			'label'   => __( 'Show Label', 'pixerex-elements' ),
			'type'    => Controls_Manager::SWITCHER,
			'default' => 'yes',
		] );

		$this->add_control( 'mark_required', [
			'label'     => __( 'Show Required Mark', 'pixerex-elements' ),
			'type'      => Controls_Manager::SWITCHER,
			'condition' => [
				'show_labels' => 'yes',
			],
		...
		$this->add_control( 'reg_button_heading', [
			'label'     => esc_html__( 'Register Button', 'pixerex-elements' ),
			'type'      => Controls_Manager::HEADING,
			'separator' => 'before',
		] );
		...
		$this->add_control( 'reg_button_text', [
			...
			'type'    => Controls_Manager::TEXT,
			'dynamic' => [ 'active' => true, ],
			'default' => __( 'Register', 'pixerex-elements' ),
		] );
	...
	protected function init_content_register_options_controls() {
		...
		$this->start_controls_section( 'section_content_register_actions', [
			'label'      => __( 'Register Form Options', 'pixerex-elements' ),
			'conditions' => $this->get_form_controls_display_condition( 'register' ),
		] );

		$this->add_control( 'register_action', [
			'label'       => __( 'Register Actions', 'pixerex-elements' ),
			'description' => __( 'You can select what should happen after a user registers successfully', 'pixerex-elements' ),
			'type'        => Controls_Manager::SELECT2,
			'multiple'    => true,
			'label_block' => true,
			'default'     => 'send_email',
			'options'     => [
				'redirect'   => __( 'Redirect', 'pixerex-elements' ),
				'auto_login' => __( 'Auto Login', 'pixerex-elements' ),
				'send_email' => __( 'Notify User By Email', 'pixerex-elements' ),
			],
		] );

		$this->add_control( 'register_redirect_url', [
			'type'          => Controls_Manager::URL,
			'label'         => __( 'Custom Redirect URL', 'pixerex-elements' ),
			'show_external' => false,
			'placeholder'   => __( 'eg. https://your-link.com/wp-admin/', 'pixerex-elements' ),
			'description'   => __( 'Please note that only your current domain is allowed here to keep your site secure.', 'pixerex-elements' ),
			'default'       => [
				'url'         => get_admin_url(),
				'is_external' => false,
		...
		$this->add_control( 'register_user_role', [
			'label'     => __( 'New User Role', 'pixerex-elements' ),
			'type'      => Controls_Manager::SELECT,
			'default'   => '',
	...
	protected function init_content_register_user_email_controls() {
		...
		$default_subject = sprintf( __( 'Thank you for registering on "%s"!', 'pixerex-elements' ), get_option( 'blogname' ) );
		$default_message = $default_subject . "\r\n\r\n";
		$default_message .= __( 'Username: [username]', 'pixerex-elements' ) . "\r\n\r\n";
		$default_message .= __( 'Password: [password]', 'pixerex-elements' ) . "\r\n\r\n";
		$default_message .= __( 'To reset your password, visit the following address:', 'pixerex-elements' ) . "\r\n\r\n";
		$default_message .= "[password_reset_link]\r\n\r\n";
		$default_message .= __( 'Please click the following address to login to your account:', 'pixerex-elements' ) . "\r\n\r\n";
		$default_message .= wp_login_url() . "\r\n";

		$this->start_controls_section( 'section_content_reg_email', [
			'label'      => __( 'Register User Email Options', 'pixerex-elements' ),
			'conditions' => [
				'relation' => 'or',
				'terms'    => [
					[
		...
		$this->add_control( 'reg_email_template_type', [
			'label'       => __( 'Email Template Type', 'pixerex-elements' ),
			'description' => __( 'Default template uses WordPress Default email template. So, please select the Custom Option to send the user proper information if you used any username field.', 'pixerex-elements' ),
			'type'        => Controls_Manager::SELECT,
			'default'     => 'default',
			'render_type' => 'none',
			'options'     => [
				'default' => __( 'WordPres Default', 'pixerex-elements' ),
				'custom'  => __( 'Custom', 'pixerex-elements' ),
			],
		] );

		$this->add_control( 'reg_email_subject', [
			'label'       => __( 'Email Subject', 'pixerex-elements' ),
			'type'        => Controls_Manager::TEXT,
		...
		$this->add_control( 'reg_email_message', [
			'label'       => __( 'Email Message', 'pixerex-elements' ),
			'type'        => Controls_Manager::WYSIWYG,
			'placeholder' => __( 'Enter Your Custom Email Message..', 'pixerex-elements' ),
		...
		$this->add_control( 'reg_email_content_note', [
			'type'            => Controls_Manager::RAW_HTML,
			'raw'             => __( '<strong>Note:</strong> You can use dynamic content in the email body like [fieldname]. For example [username] will be replaced by user-typed username. Available tags are: [password], [username], [email], [firstname],[lastname], [website], [loginurl], [password_reset_link] and [sitetitle] ', 'pixerex-elements' ),
			'content_classes' => 'elementor-panel-alert elementor-panel-alert-info',
			'condition'       => [
				'reg_email_template_type' => 'custom',
			],
		...
		$this->add_control( 'reg_email_content_type', [
			'label'       => __( 'Email Content Type', 'pixerex-elements' ),
			'type'        => Controls_Manager::SELECT,
			'default'     => 'html',
			'render_type' => 'none',
			'options'     => [
				'html'  => __( 'HTML', 'pixerex-elements' ),
				'plain' => __( 'Plain', 'pixerex-elements' ),
			],
			'condition'   => [
				'reg_email_template_type' => 'custom',
			],
	...
	protected function init_content_register_admin_email_controls() {
		/* translators: %s: Site Name */
		$default_subject = sprintf( __( '["%s"] New User Registration', 'pixerex-elements' ), get_option( 'blogname' ) );
		/* translators: %s: Site Name */
		$default_message = sprintf( __( "New user registration on your site %s", 'pixerex-elements' ), get_option( 'blogname' ) ) . "\r\n\r\n";
		$default_message .= __( 'Username: [username]', 'pixerex-elements' ) . "\r\n\r\n";
		$default_message .= __( 'Email: [email]', 'pixerex-elements' ) . "\r\n\r\n";


		$this->start_controls_section( 'section_content_reg_admin_email', [
			'label'      => __( 'Register Admin Email Options', 'pixerex-elements' ),
			'conditions' => [
				'relation' => 'or',
				'terms'    => [
					[
		...
		$this->add_control( 'reg_admin_email_template_type', [
			'label'       => __( 'Email Template Type', 'pixerex-elements' ),
			'description' => __( 'Default template uses WordPress Default Admin email template. You can customize it by choosing the custom option.', 'pixerex-elements' ),
			'type'        => Controls_Manager::SELECT,
			'default'     => 'default',
			'render_type' => 'none',
			'options'     => [
		...
		$this->add_control( 'reg_admin_email_content_note', [
			'type'            => Controls_Manager::RAW_HTML,
			'raw'             => __( '<strong>Note:</strong> You can use dynamic content in the email body like [fieldname]. For example [username] will be replaced by user-typed username. Available tags are: [username], [email], [firstname],[lastname], [website], [loginurl] and [sitetitle] ', 'pixerex-elements' ),
			'content_classes' => 'elementor-panel-alert elementor-panel-alert-info',
			'condition'       => [
				'reg_admin_email_template_type' => 'custom',
			],
	...
	protected function init_style_general_controls() {
		...
		$this->add_control( 'form_form_wrap_po_toggle', [
			'label'        => __( 'Container Box', 'pixerex-elements' ),
			'type'         => Controls_Manager::POPOVER_TOGGLE,
			'label_off'    => __( 'Default', 'pixerex-elements' ),
			'label_on'     => __( 'Custom', 'pixerex-elements' ),
			'return_value' => 'yes',
		...
		$this->add_group_control( Group_Control_Box_Shadow::get_type(), [
			'label'    => __( 'Container Box Shadow', 'pixerex-elements' ),
		...
		$this->add_control( 'eael_form_wrapper_heading', [
			'type'      => Controls_Manager::HEADING,
			'label'     => __( '---Form Wrapper---', 'pixerex-elements' ),
			'separator' => 'before',
		] );

		$this->add_responsive_control( "eael_form_width", [
			'label'           => esc_html__( 'Wrapper width', 'pixerex-elements' ),
			'type'            => Controls_Manager::SLIDER,
			'size_units'      => [
				'px',
				'rem',
		...
		$this->add_responsive_control( "eael_form_margin", [
			'label'      => __( 'Wrapper Margin', 'pixerex-elements' ),
			'type'       => Controls_Manager::DIMENSIONS,
			'size_units' => [
				'px',
				'em',
		...
		$this->add_responsive_control( "eael_form_padding", [
			'label'      => __( 'Wrapper Padding', 'pixerex-elements' ),
			'type'       => Controls_Manager::DIMENSIONS,
			'size_units' => [
				'px',
				'em',
		...
		$this->add_control( "eael_form_border_radius", [
			'label'      => __( 'Wrapper Border Radius', 'pixerex-elements' ),
			'type'       => Controls_Manager::DIMENSIONS,
			'size_units' => [
				'px',
				'%',
		...
		$this->add_control( 'eael_form_input_container', [
			'type'      => Controls_Manager::HEADING,
			'label'     => __( '---Form Style---', 'pixerex-elements' ),
			'separator' => 'before',
		] );
		...
		$this->add_responsive_control( "eael_form_ic_margin", [
			'label'      => __( 'Form Margin', 'pixerex-elements' ),
			'type'       => Controls_Manager::DIMENSIONS,
			'size_units' => [
				'px',
				'em',
		...
		$this->add_responsive_control( "eael_form_ic_padding", [
			'label'      => __( 'Form Padding', 'pixerex-elements' ),
			'type'       => Controls_Manager::DIMENSIONS,
			'size_units' => [
				'px',
				'em',
		...
		$this->add_group_control( Group_Control_Box_Shadow::get_type(), [
			'label'    => __( 'Form Wrapper Shadow', 'pixerex-elements' ),
		...
		$this->add_group_control( Group_Control_Box_Shadow::get_type(), [
			'label'    => __( 'Form Shadow', 'pixerex-elements' ),
	...
	protected function init_style_header_content_controls( $form_type = 'login' ) {
		$this->start_controls_section( "section_style_{$form_type}_header_content", [
			'label'      => sprintf( __( '%s Form Header', 'pixerex-elements' ), ucfirst( $form_type ) ),
		...
		$this->add_control( "{$form_type}_fhc_po_toggle", [
			'label'        => __( 'Header Content', 'pixerex-elements' ),
			'type'         => Controls_Manager::POPOVER_TOGGLE,
			'label_off'    => __( 'Default', 'pixerex-elements' ),
			'label_on'     => __( 'Custom', 'pixerex-elements' ),
			'return_value' => 'yes',
		...
		$this->add_responsive_control( "{$form_type}_fhc_width", [
			'label'      => esc_html__( 'Header width', 'pixerex-elements' ),
			'type'       => Controls_Manager::SLIDER,
			'size_units' => [
				'px',
				'rem',
		...
		$this->add_responsive_control( "{$form_type}_fhc_height", [
			'label'      => esc_html__( 'Header height', 'pixerex-elements' ),
			'type'       => Controls_Manager::SLIDER,
			'size_units' => [
				'px',
				'rem',
		...
		$this->add_responsive_control( "{$form_type}_fhc_margin", [
			'label'      => __( 'Header Margin', 'pixerex-elements' ),
			'type'       => Controls_Manager::DIMENSIONS,
			'size_units' => [
				'px',
				'em',
		...
		$this->add_responsive_control( "{$form_type}_fhc_padding", [
			'label'      => __( 'Header Padding', 'pixerex-elements' ),
			'type'       => Controls_Manager::DIMENSIONS,
			'size_units' => [
				'px',
				'em',
		...
		$this->add_control( "{$form_type}_form_img_po_toggle", [
			'label'        => __( 'Form Illustration', 'pixerex-elements' ),
			'type'         => Controls_Manager::POPOVER_TOGGLE,
			'label_off'    => __( 'Default', 'pixerex-elements' ),
			'label_on'     => __( 'Custom', 'pixerex-elements' ),
			'return_value' => 'yes',
		...
		$this->end_popover();
		$this->add_group_control( Group_Control_Box_Shadow::get_type(), [
			'label'    => __( 'Illustration Shadow', 'pixerex-elements' ),
		...
		$this->add_control( "{$form_type}_form_logo_po_toggle", [
			'label'        => __( 'Form Logo', 'pixerex-elements' ),
			'type'         => Controls_Manager::POPOVER_TOGGLE,
			'label_off'    => __( 'Default', 'pixerex-elements' ),
			'label_on'     => __( 'Custom', 'pixerex-elements' ),
			'return_value' => 'yes',
		...
		$this->end_popover();
		$this->add_group_control( Group_Control_Box_Shadow::get_type(), [
			'label'    => __( 'Logo Shadow', 'pixerex-elements' ),
		...
		$this->add_group_control( Group_Control_Typography::get_type(), [
			...
			'label'    => __( 'Title Typography', 'pixerex-elements' ),
			'selector' => $title_selector,
		] );
		...
		$this->add_group_control( Group_Control_Typography::get_type(), [
			...
			'label'    => __( 'Subtitle Typography', 'pixerex-elements' ),
			'selector' => $subtitle_selector,
		] );
	...
	protected function init_style_input_fields_controls() {
		$this->start_controls_section( 'section_style_form_fields', [
			'label' => __( 'Form Fields', 'pixerex-elements' ),
			'tab'   => Controls_Manager::TAB_STYLE,
		] );
		...
		$this->add_control( 'eael_form_input_fields_heading', [
			'type'  => Controls_Manager::HEADING,
			'label' => __( 'Form Input Fields', 'pixerex-elements' ),
		] );
		...
		$this->add_control( 'eael_form_rm_fields_heading', [
			'type'      => Controls_Manager::HEADING,
			'label'     => __( 'Remember Me Field', 'pixerex-elements' ),
			'separator' => 'before',
		] );
		...
		$this->add_responsive_control( "ph_align", [
			'label'     => __( 'Text Alignment', 'pixerex-elements' ),
			'type'      => Controls_Manager::CHOOSE,
			'options'   => [
				'left'   => [
					'title' => __( 'Left', 'pixerex-elements' ),
		...
		$this->add_control( 'eael_form_label_colors_heading', [
			'type'      => Controls_Manager::HEADING,
			'label'     => __( 'Colors & Border', 'pixerex-elements' ),
			'separator' => 'before',
		] );
	...
	protected function init_style_input_labels_controls() {
		$this->start_controls_section( 'section_style_form_labels', [
			'label' => __( 'Form Labels', 'pixerex-elements' ),
			'tab'   => Controls_Manager::TAB_STYLE,
		] );
		...
		$this->add_control( 'eael_form_label_c_po_toggle', [
			'label'        => __( 'Colors', 'pixerex-elements' ),
			'type'         => Controls_Manager::POPOVER_TOGGLE,
			'label_off'    => __( 'Default', 'pixerex-elements' ),
			'label_on'     => __( 'Custom', 'pixerex-elements' ),
			'return_value' => 'yes',
		...
		$this->add_control( 'rmark_po_toggle', [
			'label'     => __( 'Required Mark Style', 'pixerex-elements' ),
			'type'      => Controls_Manager::POPOVER_TOGGLE,
			'condition' => [
				'show_labels'   => 'yes',
				'mark_required' => 'yes',
		...
		$this->add_control( 'rmark_sign', [
			'label'       => __( 'Mark Sign', 'pixerex-elements' ),
			'type'        => Controls_Manager::TEXT,
			'default'     => '*',
		...
		$this->add_responsive_control( "rmark_valign", [
			'label'     => esc_html__( 'Vertical Alignment', 'pixerex-elements' ),
			'type'      => Controls_Manager::SLIDER,
			'range'     => [
				'px' => [
					'min'  => - 50,
		...
		$this->add_responsive_control( "rmark_halign", [
			'label'     => esc_html__( 'Horizontal Alignment', 'pixerex-elements' ),
			'type'      => Controls_Manager::SLIDER,
			'range'     => [
				'px' => [
					'min'  => - 50,
		...
		$this->add_control( 'lpv_po_toggle', [
			'label'     => __( 'Password Visibility Style', 'pixerex-elements' ),
			'type'      => Controls_Manager::POPOVER_TOGGLE,
			'condition' => [
				'password_toggle' => 'yes',
			],
		...
		$this->add_control( "lvp_open_color", [
			'label'     => __( 'Open Eye Color', 'pixerex-elements' ),
			'type'      => Controls_Manager::COLOR,
			'selectors' => [
		...
		$this->add_control( "lvp_close_color", [
			'label'     => __( 'Close Eye Color', 'pixerex-elements' ),
			'type'      => Controls_Manager::COLOR,
			'selectors' => [
	...
	protected function _init_button_style( $button_type = 'login' ) {
		$this->start_controls_section( "section_style_{$button_type}_btn", [
			'label'      => sprintf( __( '%s Button', 'pixerex-elements' ), ucfirst( $button_type ) ),
			'tab'        => Controls_Manager::TAB_STYLE,
		...
		$this->add_responsive_control( "{$button_type}_btn_d_type", [
			'label'     => __( 'Display as', 'pixerex-elements' ),
			'type'      => Controls_Manager::SELECT,
			'options'   => [
				'row'    => __( 'Inline', 'pixerex-elements' ),
				'column' => __( 'Block', 'pixerex-elements' ),
			],
			'default'   => 'row',
			'selectors' => [
		...
		$this->add_responsive_control( "{$button_type}_btn_jc", [
			'label'     => __( 'Justify Content', 'pixerex-elements' ),
			'type'      => Controls_Manager::SELECT,
			'options'   => [
				'flex-start'    => __( 'Start', 'pixerex-elements' ),
				'flex-end'      => __( 'End', 'pixerex-elements' ),
				'center'        => __( 'Center', 'pixerex-elements' ),
				'space-between' => __( 'Space Between', 'pixerex-elements' ),
				'space-around'  => __( 'Space Around', 'pixerex-elements' ),
				'space-evenly'  => __( 'Space Evenly', 'pixerex-elements' ),
			],
		...
		$this->add_responsive_control( "{$button_type}_btn_width", [
			'label'      => esc_html__( 'Button width', 'pixerex-elements' ),
			'type'       => Controls_Manager::SLIDER,
			'size_units' => [
				'px',
				'%',
		...
		$this->add_responsive_control( "{$button_type}_btn_height", [
			'label'      => esc_html__( 'Button Height', 'pixerex-elements' ),
			'type'       => Controls_Manager::SLIDER,
			'size_units' => [
				'px',
				'%',
	...
	protected function _init_recaptcha_style( $form_type = 'login' ) {
		$this->start_controls_section( "section_style_{$form_type}_rc", [
			'label'     => sprintf( __( '%s Form reCAPTCHA', 'pixerex-elements' ), ucfirst( $form_type ) ),
			'tab'       => Controls_Manager::TAB_STYLE,
			'condition' => [
		...
		$this->add_control( "{$form_type}_rc_theme", [
			'label'   => __( 'Theme', 'pixerex-elements' ),
			'type'    => Controls_Manager::SELECT,
			'options' => [
				'light' => __( 'Light', 'pixerex-elements' ),
				'dark'  => __( 'Dark', 'pixerex-elements' ),
			],
			'default' => 'light',
		] );
		...
		$this->add_control( "{$form_type}_rc_size", [
			...
			'type'    => Controls_Manager::SELECT,
			'options' => [
				'normal'  => __( 'Normal', 'pixerex-elements' ),
				'compact' => __( 'Compact', 'pixerex-elements' ),
			],
			'default' => 'normal',
		] );
	...
	protected function _init_link_style( $form_type = 'login' ) {
		$form_name = 'login' === $form_type ? __( 'Register', 'pixerex-elements' ) : __( 'Login', 'pixerex-elements' );
		$this->start_controls_section( "section_style_{$form_type}_link", [
			'label'     => sprintf( __( '%s Link', 'pixerex-elements' ), ucfirst( $form_name ) ),
			'tab'       => Controls_Manager::TAB_STYLE,
			'condition' => [
		...
		$this->add_control( "{$form_type}_link_style_notice", [
			'type'            => Controls_Manager::RAW_HTML,
			'raw'             => sprintf( __( 'Here you can style the %s link displayed on the %s Form', 'pixerex-elements' ), $form_name, ucfirst( $form_type ) ),
			'content_classes' => 'elementor-panel-alert elementor-panel-alert-info',
		] );
		...
		$this->add_responsive_control( "{$form_type}_link_ai", [
			'label'     => __( 'Align Items', 'pixerex-elements' ),
			'type'      => Controls_Manager::SELECT,
			'options'   => [
				'flex-start'   => __( 'Start', 'pixerex-elements' ),
				'flex-end'     => __( 'End', 'pixerex-elements' ),
				'center'       => __( 'Center', 'pixerex-elements' ),
				'stretch'      => __( 'Stretch', 'pixerex-elements' ),
				'baseline'     => __( 'Baseline', 'pixerex-elements' ),
				'space-evenly' => __( 'Space Evenly', 'pixerex-elements' ),
			],
			'default'   => 'center',
			'condition' => [
		...
		$this->add_responsive_control( "{$form_type}_link_wrap_width", [
			'label'      => esc_html__( 'Link Container width', 'pixerex-elements' ),
			'type'       => Controls_Manager::SLIDER,
			'size_units' => [
				'px',
				'%',
		...
		$this->add_responsive_control( "{$form_type}_link_width", [
			'label'      => esc_html__( 'Link width', 'pixerex-elements' ),
			'type'       => Controls_Manager::SLIDER,
			'size_units' => [
				'px',
				'%',
		...
		$this->add_responsive_control( "{$form_type}_link_height", [
			'label'      => esc_html__( 'Link Height', 'pixerex-elements' ),
			'type'       => Controls_Manager::SLIDER,
			'size_units' => [
				'px',
				'%',
	...
	protected function print_login_form() {
		if ( $this->should_print_login_form ) {
			...
			$reg_link_text   = ! empty( $this->get_settings( 'registration_link_text' ) ) ? $this->get_settings( 'registration_link_text' ) : __( 'Register', 'pixerex-elements' );
			$parts           = explode( "\n", $reg_link_text );
			...
			$u_label = $u_ph = __( 'Username or Email Address', 'pixerex-elements' );
			$p_label = $p_ph = __( 'Password', 'pixerex-elements' );
			...
			$lp_text = ! empty( $this->ds['lost_password_text'] ) ? $this->ds['lost_password_text'] : __( 'Forgot password?', 'pixerex-elements' );
            ...
            <section id="eael-login-form-wrapper" class="<?php echo esc_attr( $default_hide_class ); ?>" data-recaptcha-theme="<?php echo esc_attr( $rc_theme ); ?>" data-recaptcha-size="<?php echo esc_attr( $rc_size ); ?>">
                <div class="eael-login-form-wrapper eael-lr-form-wrapper style-2">
					...
					if ( $show_logout_link && is_user_logged_in() && ! $this->in_editor ) {
						...
						$logged_in_msg = sprintf( __( 'You are already logged in as %s. ', 'pixerex-elements' ), wp_get_current_user()->display_name );
						printf( '%1$s   (<a href="%2$s">%3$s</a>)', $logged_in_msg, esc_url( wp_logout_url() ), __( 'Logout', 'pixerex-elements' ) );
					} else {
                                ...
                                <div class="eael-forever-forget eael-lr-form-group">
									<?php if ( $show_rememberme ) { ?>
                                        <p class="forget-menot">
                                            ...
                                            <label for="rememberme" class="eael-checkbox-label check-rememberme"><?php esc_html_e( 'Remember Me', 'pixerex-elements' ); ?></label>
                                        </p>
									<?php }
									if ( $show_lp ) {
	...
	protected function print_register_form() {
		if ( $this->should_print_register_form ) {
			...
			$f_labels            = [
				'email'            => __( 'Email', 'pixerex-elements' ),
				'password'         => __( 'Password', 'pixerex-elements' ),
				'confirm_password' => __( 'Confirm Password', 'pixerex-elements' ),
				'user_name'        => __( 'Username', 'pixerex-elements' ),
				'first_name'       => __( 'First Name', 'pixerex-elements' ),
				'last_name'        => __( 'Last Name', 'pixerex-elements' ),
				'website'          => __( 'Website', 'pixerex-elements' ),
			...
			$lgn_link_text   = ! empty( $this->get_settings( 'login_link_text' ) ) ? $this->get_settings( 'login_link_text' ) : __( 'Login', 'pixerex-elements' );
	...
	protected function print_form_header( $form_type = 'login' ) {
        ...
        <div class="lr-form-header header-<?php echo esc_attr( $this->form_logo_pos ); ?>">
			<?php if ( ! empty( $this->form_logo ) ) { ?>
                <div class="form-logo">
                    <img src="<?php echo esc_attr( esc_url( $this->form_logo ) ); ?>" alt="<?php esc_attr_e( 'Form Logo Image', 'pixerex-elements' ); ?>">
                </div>
			<?php } ?>
	...
	protected function print_error_for_repeated_fields( $repeated_fields ) {
		if ( ! empty( $repeated_fields ) ) {
            ...
            <p class='eael-register-form-error elementor-alert elementor-alert-warning'>
				...
				printf( __( 'Error! you seem to have added %s field in the form more than once.', 'pixerex-elements' ), $error_fields );
				?>
            </p>
			<?php
			return true;
	...
	protected function print_error_for_missing_email_field( $email_exist ) {
		if ( empty( $email_exist ) ) {
            ...
            <p class='eael-register-form-error elementor-alert elementor-alert-warning'>
				...
				printf( __( 'Error! It is required to use %s field.', 'pixerex-elements' ), '<strong>Email</strong>' );
				?>
            </p>
			<?php
			return true;
	...
	protected function print_error_for_missing_password_field( $password_exist, $confirm_pass_exist ) {
		if ( empty( $password_exist ) && ! empty( $confirm_pass_exist ) ) {
            ...
            <p class='eael-register-form-error elementor-alert elementor-alert-warning'>
				<?php
				/* translators: %s: Error String */
				printf( __( 'Error! It is required to use %s field with %s Field.', 'pixerex-elements' ), '<strong>Password</strong>', '<strong>Password Confirmation</strong>' );
				?>
            </p>
			<?php
			return true;
